...
export default function LoginScreen({ navigation }) {
  ...
  const { setUser } = useContext(AuthContext); // assume AuthContext provides setUser
  ...
  async function onLogin() {
    ...
    try {
      ...
      // Update AuthContext (so your app knows user is logged in)
      if (setUser) {
        setUser({ user, profile, session });
      }

      // Use replace instead of reset to avoid navigation.reset issues
      // replace preserves a clean navigation stack and avoids dispatch/reset mismatches
